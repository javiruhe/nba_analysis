...
```{r}
dataReDraft <- read_csv("dataReDraft.csv") %>% as_tibble()
dataPerformance <- read_csv("dataPerformance.csv") %>% as_tibble()
```

```{r, echo=FALSE, }
top_redraft <- dataReDraft %>%
  filter(year == 2018) %>% 
...
hchart(top_redraft, "scatter", hcaes(x = as.numeric(overall_pick), y = newPick, group = rank)) %>%
  hc_title(text = "Realidad vs. Valor Esperado (Re-Draft)") %>%
...
```{r}
plot_ly(dataPerformance %>% distinct(player, .keep_all = TRUE), 
...
```{r}
df_heat <- dataPerformance %>%
  filter(year == "2016", rank %in% c("Generational")) %>%
